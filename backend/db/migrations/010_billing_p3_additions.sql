...
-- ─────────────────────────────────────────────────────────────────────────────
-- P3-T12: Founding Family seed — camarata@yourflock.com billing_exempt forever
-- ─────────────────────────────────────────────────────────────────────────────
DO $$
DECLARE
    founder_id UUID;
...
BEGIN
    SELECT id INTO founder_id FROM subscribers WHERE email = 'camarata@yourflock.com';
    ...
    IF founder_id IS NOT NULL THEN
        ...
        IF founder_plan_id IS NOT NULL THEN
            ...
            ON CONFLICT (subscriber_id) DO UPDATE SET
                ...
                updated_at = NOW();
            RAISE NOTICE 'Founding family camarata@yourflock.com seeded (billing_exempt=true, plan=founding)';
        END IF;
    ELSE
