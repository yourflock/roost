...
<script lang="ts">
	...
	let actorId = $derived(data.filters.actor_id);
	let action = $derived(data.filters.action);
	let resourceType = $derived(data.filters.resource_type);
	let dateFrom = $derived(data.filters.date_from);
	let dateTo = $derived(data.filters.date_to);
...
<div class="p-6 max-w-7xl mx-auto">
	<div class="mb-8">
		...
		<p class="text-slate-400 mt-1">
			Immutable trail of all admin and subscriber actions — {data.total.toLocaleString()} total entries
		</p>
	</div>
	...
	<div class="bg-slate-800 border border-slate-700 rounded-xl p-5 mb-6">
		<div class="grid grid-cols-2 gap-4 md:grid-cols-5">
			<div>
				<label class="block text-xs text-slate-400 mb-1" for="actorId">Actor ID (UUID)</label>
				<input
					type="text"
					id="actorId"
					bind:value={actorId}
			...
			<div>
				<label class="block text-xs text-slate-400 mb-1" for="actionFilter">Action</label>
				<input
					type="text"
					id="actionFilter"
					bind:value={action}
			...
			<div>
				<label class="block text-xs text-slate-400 mb-1" for="resourceType">Resource Type</label>
				<select
					id="resourceType"
			...
			<div>
				<label class="block text-xs text-slate-400 mb-1" for="dateFrom">From</label>
				<input
					type="datetime-local"
					id="dateFrom"
					bind:value={dateFrom}
			...
			<div>
				<label class="block text-xs text-slate-400 mb-1" for="dateTo">To</label>
				<input
					type="datetime-local"
					id="dateTo"
					bind:value={dateTo}
		...
		<div class="mt-3 flex gap-2">
			<button
				...
				class="px-4 py-2 bg-roost-600 hover:bg-roost-500 text-white rounded-lg text-sm font-medium transition-colors"
			>
				Apply Filters
			</button>
			<button
			...
			<table class="w-full text-sm">
				<thead>
					<tr class="border-b border-slate-700 text-left">
						<th class="px-4 py-3 text-xs font-medium text-slate-400 uppercase tracking-wider"
							>When</th
						>
						<th class="px-4 py-3 text-xs font-medium text-slate-400 uppercase tracking-wider"
							>Actor</th
						...
						>
						<th class="px-4 py-3 text-xs font-medium text-slate-400 uppercase tracking-wider w-8"
						></th>
					</tr>
				</thead>
						...
						>
							...
							<td class="px-4 py-3">
								<span class="inline-flex items-center gap-1.5">
									<span
										class="px-2 py-0.5 rounded-full text-xs font-medium {actorBadgeClass(
											entry.actor_type
										)}">{entry.actor_type}</span
									>
									{#if entry.actor_id}
										<span class="text-slate-500 text-xs font-mono"
											>{entry.actor_id.slice(0, 8)}…</span
										>
									{/if}
								</span>
							</td>
							...
							<tr class="bg-slate-900">
								<td colspan="6" class="px-4 py-3">
									<div class="text-xs space-y-1">
										<div>
											<span class="text-slate-400">Full Actor ID:</span>
											<span class="text-slate-200 font-mono">{entry.actor_id ?? '—'}</span>
										</div>
										<div>
											<span class="text-slate-400">Full Resource ID:</span>
											<span class="text-slate-200 font-mono">{entry.resource_id ?? '—'}</span>
										</div>
										{#if entry.user_agent}
											<div>
												<span class="text-slate-400">User-Agent:</span>
												<span class="text-slate-300">{entry.user_agent}</span>
											</div>
										{/if}
										{#if Object.keys(entry.details ?? {}).length > 0}
											<div>
												...
												<pre
													class="mt-1 bg-slate-800 rounded p-2 text-slate-200 overflow-x-auto">{JSON.stringify(
														entry.details,
														null,
														2
													)}</pre>
