<script lang="ts">
	...
	import { locale, LOCALE_OPTIONS, setLocale, type Locale } from '$lib/i18n';
...
<div class="min-h-screen flex flex-col bg-slate-900" dir={dir}>
	<!-- Header -->
	<header class="border-b border-slate-800 bg-slate-900/95 backdrop-blur sticky top-0 z-50">
		<div class="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
			...
			<a href="/" class="flex items-center gap-2 text-white font-semibold text-lg">
				<div class="w-8 h-8 bg-roost-500 rounded-lg flex items-center justify-center text-white font-bold text-sm">R</div>
				<span>Roost</span>
			</a>
			...
			<nav class="hidden md:flex items-center gap-6 text-sm">
				{#each navLinks as link}
					<a href={link.href} class="text-slate-400 hover:text-white transition-colors">{link.label}</a>
				{/each}
				{#if isLoggedIn}
