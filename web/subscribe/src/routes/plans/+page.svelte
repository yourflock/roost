<script lang="ts">
	...
	// P14-T03/T06: Localized currency display using Intl.NumberFormat.
	// If the plan has a regional_price attached (subscriber has region set),
	// display the regional price and currency. Otherwise fall back to plan defaults.
	function getDisplayCurrency(plan: Plan): string {
		return (plan as PlanWithRegion).regional_price?.currency?.toUpperCase() ?? 'USD';
	}
...
<div class="max-w-5xl mx-auto px-4 py-16">
	<div class="text-center mb-12">
		...
		<p class="text-slate-400 mb-8">All plans include the full channel lineup. Upgrade or cancel anytime.</p>

		<!-- Billing toggle -->
		<div class="inline-flex items-center gap-3 bg-slate-800 rounded-full p-1.5">
			<button
				on:click={() => (annual = false)}
				class="px-4 py-1.5 rounded-full text-sm font-medium transition-colors {!annual ? 'bg-roost-500 text-white' : 'text-slate-400 hover:text-white'}"
			>
				Monthly
			</button>
			<button
			...
			>
				Annual
				<span class="ml-1 text-xs {annual ? 'text-roost-200' : 'text-green-400'}">Save 2 months</span>
			</button>
		</div>
	</div>
	...
	<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
		{#each data.plans as plan}
			<div class="card relative flex flex-col {plan.is_popular ? 'border-roost-500 ring-1 ring-roost-500/50' : ''}">
				{#if plan.is_popular}
					<div class="absolute -top-3.5 left-1/2 -translate-x-1/2">
						<span class="bg-roost-500 text-white text-xs font-semibold px-4 py-1 rounded-full">Most Popular</span>
					</div>
				{/if}
				...
				<div class="flex-1">
					...
					{#if annual}
						...
						<p class="text-xs text-green-400 mb-4">Save ${(savings(plan) / 100).toFixed(2)} vs monthly</p>
					{:else}
					...
					<ul class="space-y-2 mb-6">
						{#each plan.features as feature}
							<li class="flex items-start gap-2 text-sm text-slate-300">
								<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-roost-400 mt-0.5 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
