...
func (s *Server) Routes() http.Handler {
	...
	// OSG.1.002 — Stream source registry CRUD
	r.Post("/sports/sources", s.handleCreateSource)
	r.Get("/sports/sources", s.handleListSources)
	r.Get("/sports/sources/{id}", s.handleGetSource)
	r.Delete("/sports/sources/{id}", s.handleDeleteSource)
	r.Post("/sports/sources/{id}/refresh", s.handleRefreshSource)
	r.Get("/sports/sources/{id}/health", s.handleGetSourceHealth)

	// OSG.3.001 — Stream routing
	r.Get("/sports/events/{id}/stream", s.handleGetStream)

	// OSG.3.002 — Roost Boost pool
	r.Post("/sports/roost-boost/join", s.handleRoostBoostJoin)
	r.Delete("/sports/roost-boost/leave", s.handleRoostBoostLeave)
	r.Get("/sports/roost-boost/stats", s.handleRoostBoostStats)
