...
// ─── Stream source registry models ──────────────────────────────────────────

// StreamSource represents a registered IPTV stream source in sports_stream_sources.
type StreamSource struct {
	ID                 string     `json:"id"`
	Name               string     `json:"name"`
	SourceType         string     `json:"source_type"`
	M3UURL             *string    `json:"m3u_url,omitempty"`
	RoostFamilyID      *string    `json:"roost_family_id,omitempty"`
...
// SourceChannel represents one channel entry in a source's M3U playlist.
type SourceChannel struct {
	...
	MatchedLeagueID  *string    `json:"matched_league_id,omitempty"`
	MatchConfidence  float64    `json:"match_confidence"`
	MatchConfirmed   bool       `json:"match_confirmed"`
	CreatedAt        time.Time  `json:"created_at"`
}

// GameSourceAssignment represents an active source assignment for a game.
type GameSourceAssignment struct {
	ID              string    `json:"id"`
...
// RawChannel is parsed from an M3U playlist line pair (#EXTINF + URL).
